...
class Solution {
    public int removeElement(int[] nums, int val) {
        int swapIdx = nums.length - 1; // 该指针用于划分交换空间和新数组
        int scanIdx = 0; // 该指针用于扫描val元素

        while (scanIdx <= swapIdx) {
            if (nums[scanIdx] == val) { // 扫描到值为val的元素
                swap(nums, scanIdx, swapIdx); // 将val元素交换到数组的末尾
                swapIdx--; // 只移动交换指针，因为交换来的元素仍然有可能是val
            } else {
                scanIdx++;
            }
        }
